$lines = Get-Content -Path "veiculo.html" -Encoding UTF8
$header = $lines[0..64]
$footer = $lines[448..($lines.Count-1)]

for ($i=0; $i -lt $header.Count; $i++) {
    if ($header[$i] -match '<link rel="stylesheet" href="styles.css" />') {
        $header = $header[0..$i] + '    <link rel="stylesheet" href="veiculo.css" />' + $header[($i+1)..($header.Count-1)]
        break
    }
}

for ($i=0; $i -lt $footer.Count; $i++) {
    if ($footer[$i] -match '<script src="script.js"></script>') {
        $footer[$i] = $footer[$i] -replace 'script.js', 'veiculo.js'
    }
}
...
$newLines | Out-File -FilePath "veiculo.html" -Encoding UTF8
